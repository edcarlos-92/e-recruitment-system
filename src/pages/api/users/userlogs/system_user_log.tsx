  ...
  if (req.method === 'PUT') {
    ...
    await helper.checkPermission(req, "v", async function (isPermited: any) {
      if (isPermited) {
        ...
        //const getMomentDate = Moment(Date.now()).format("ll")
        const loginDateTime = `${Moment(date).format("ll")} at ${Time(date)}`;
        try {
          if (!id) {
          ...
          const system_user: any = await authQuery(`UPDATE system_users SET  login = ? WHERE id = ? `, [loginDateTime, id])
          //const system_user:any = await authQuery(`UPDATE system_users SET  login = ? WHERE id = ? `,[Date.now(),id])
